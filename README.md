...
33. **Vue : désignation complète et DLC visibles sur chaque ligne** : `construireLigneArticleVue()` a été restructurée — le code article et la désignation sont chacun sur leur propre ligne (la désignation n'est plus jamais tronquée avec "…", elle passe à la ligne autant que nécessaire), et un badge "DLC : MM/AAAA" apparaît si elle est renseignée (rouge si dépassée, comparaison au niveau du mois comme la règle 11), à côté du badge "Stock" existant. Les actions 🏷️/🗑️ sont regroupées à droite, en colonne.
34. **Accueil : sélection d'emplacement en chips (zone/façade/étage)** : les anciens `<select>` (zone, étage) et le groupe de radios classique (façade) ont été remplacés par trois groupes de chips à sélection unique (`input[name="allee"]`, `input[name="facade"]`, `input[name="etage"]`, stylées comme les chips de filtre déjà utilisées ailleurs). Fonctions utilitaires génériques dans `ui.js` : `selectionnerChip(name, valeur)` / `valeurChip(name)` / `reinitialiserChips(name)` / `activerChips(name, actif)` / `initChipsRadio(name, callback)` — utilisées partout où l'emplacement est lu ou modifié (formulaire principal, restauration de navigation au démarrage règle 21, retour depuis la Table, bouton Accueil/réinitialiser). Le comportement reste identique à avant (étage grisé et vidé si façade = Sol, zone Table masque façade/étage) ; seul le composant visuel change.
