...
16. **Export CSV du stock (bouton "📦 Stock")** : génère un vrai fichier `.csv` (pas un `.xlsx`) listant `Code article`, `Désignation`, `Stock` (= `stockTheorique` du catalogue), trié par code article. Voir `exporterStockCSV()` dans `export.js`.
