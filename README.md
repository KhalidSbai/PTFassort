...
14. **Pied de page sur les étiquettes PDF** : chaque page imprimée affiche "Plateforme SIDI GHANEM" en petit, centré en bas (`.etiquette-pied` dans `style.css`).
15. **Export CSV du stock (bouton "📦 Stock")** : génère un vrai fichier `.csv` (pas un `.xlsx`) listant `Code article`, `Désignation`, `Stock` (= `stockTheorique` du catalogue), trié par code article. Voir `exporterStockCSV()` dans `export.js`.
16. **Zone spéciale "Table"** : sélectionnable dans le menu déroulant "Zone" (à la place d'une allée 1-4), elle **n'a ni façade, ni étage, ni cellule** — c'est un espace de dépôt unique. La choisir masque les champs Façade/Étage et ouvre directement l'écran d'ajout d'articles (pas de grille de 18 cellules). En interne, `allee: 'Table'` (chaîne, pas un nombre) avec `facade`/`etage`/`cellule` toujours `null` ; `estZoneTable()` dans `utils.js` centralise cette détection et est utilisée dans `db.js`, `ui.js` et `export.js` partout où l'allée est traitée comme un nombre. La zone Table apparaît dans la Vue (affichage à plat, sans sous-niveaux), peut recevoir un article déplacé depuis n'importe quelle cellule (et inversement), et peut être vidée en masse comme les autres zones. **Sur les étiquettes PDF, le champ "Emplacement" est automatiquement masqué** pour les articles qui viennent de la Table (`genererPDFEtiquettes()` dans `ui.js`).
17. **Stock affiché et suppression individuelle dans la Vue** : chaque ligne d'article de l'écran "Vue" affiche désormais son stock réel (`Stock : X`) s'il est renseigné, et un bouton 🗑️ permet de supprimer cette occurrence précise directement depuis la Vue (avec confirmation), sans devoir rouvrir la cellule concernée. Voir `construireLigneArticleVue()` dans `ui.js`, réutilisée pour toutes les zones (y compris la Table).
18. **Quantité la plus fréquente par palette + fiabilité (écran "📦 Stock")** : le bouton "📦 Stock" de l'en-tête ouvre un écran listant chaque article avec sa quantité de palette la plus souvent comptée et le pourcentage de fiabilité associé (`nb de palettes avec cette quantité / nb total de palettes de l'article × 100`), affiché `10 (75 %)`. **Rien de ce calcul n'est jamais stocké** : `calculerQuantiteFrequente()` dans `utils.js` est une fonction pure recalculée à chaque ouverture de l'écran (ou recherche) à partir des `stockReel` actuellement enregistrés sur toutes les occurrences de l'article — après correction d'une quantité via 🏷️, rouvrir cet écran reflète toujours l'état à jour, jamais une ancienne valeur. En cas d'égalité entre plusieurs quantités, la plus petite est retenue (choix déterministe). Un bouton "📤 Exporter en CSV" dans cet écran télécharge le même calcul via `exporterStockCSV()` dans `export.js`. Ce CSV utilise le **point-virgule comme délimiteur** (`XLSX.utils.sheet_to_csv(feuille, { FS: ';' })`, téléchargé via `telechargerBlob()` avec un BOM UTF-8 pour un bon affichage des accents dans Excel) et contient, dans l'ordre : `Code article`, `Désignation`, `Stock théorique`, `Stock réel` (somme des `stockReel` de toutes les palettes de l'article — vide si aucune palette comptée), `Quantité la plus fréquente / palette`, `Fiabilité (%)`, `Palettes comptées`.
19. **Filtre "stock théorique ≤ 0" dans la Vue** : une case à cocher "⚠️ Afficher uniquement les articles avec un stock théorique ≤ 0" dans l'écran "Vue" filtre la liste (combinable avec le filtre par zone et la recherche) pour ne montrer que les occurrences dont l'article a un `stockTheorique` négatif ou nul — utile pour identifier après un import théorique les articles déjà placés en cellule qui n'ont plus de stock. Chaque ligne y garde ses boutons 🏷️ (modifier stock/DLC/code-barres, ouvre la même modale que dans le détail de cellule) et 🗑️ (supprimer cette occurrence), pour corriger ou nettoyer directement depuis cette vue filtrée. La case est décochée à chaque nouvelle ouverture de la Vue. Voir `checkbox-stock-negatif` dans `renderContenuVueZone()` (`ui.js`).
